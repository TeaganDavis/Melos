    ...
    <?php 
        ...
        function retrieveData($playlistId) {
            ...
            $PlaylistWithSongs = $DBConnection->query("
                SELECT p.playlist_id, p.user_id, p.playlist_name, p.playlist_desc, p.privacy,
            ...
            $PlaylistCreatorResult = $DBConnection->query("
                SELECT p.*, u.username
                FROM playlists p
                LEFT JOIN users u ON p.user_id = u.user_id
                WHERE p.playlist_id = $playlistId");
            $PlaylistCreator = $PlaylistCreatorResult->fetch_assoc();
            ...
            if ($PlaylistWithSongs->num_rows > 0){
                ...
                displayPlaylistInfo($playlistInfo, $songs, $PlaylistCreator);
            } else {
                echo "<h2>Playlist is empty.</h2>";
            }
        }

        function displayPlaylistInfo($playlistInfo, $songs, $PlaylistCreator){
            echo "<div id='playlistHead'>
                <h1>". $playlistInfo['playlist_name'] ."</h1>
                <h2> Created by: ". $PlaylistCreator['username'] ."</h2>
                <h3>". $playlistInfo['playlist_desc'] ."</h3>
                <h4>". $playlistInfo['privacy'] ."</h4>
            </div>";
            ...
            foreach ($songs as $songRow) {
                ...
                <tr>
                    <td>". $songRow['title'] ."</td>
                    <td>". $songRow['artist'] ."</td>
                    <td>". $songRow['album'] ."</td>
                    <td>". $songRow['song_length'] ."</td>
                    <td>". $songRow['release_date'] ."</td>
                    <td>". $songRow['user_rating'] ."</td>
                </tr>";
            }
...
<body>
    ...
    <div id="content">
        <a style="padding-top: 30px;" href="../pages/usercollection.php"> < Back </a>
    </div>
